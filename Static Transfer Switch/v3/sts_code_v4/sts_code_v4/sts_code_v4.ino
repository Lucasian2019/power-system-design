...
void updateFrequency()
{
  ...
  // SOURCE A
  if (pA >= 14000UL && pA <= 23000UL)
  {
    float newFreqA = 1000000.0 / (float)pA;

    frequencyBufferA[freqIndexA] = newFreqA;
    freqIndexA++;

    if (freqIndexA >= FREQUENCY_SAMPLES)
    {
      freqIndexA = 0;
    }
    ...
    {
      sumA += frequencyBufferA[i];
    }
    freqA = sumA / FREQUENCY_SAMPLES;
  }

  // SOURCE B
  if (pB >= 14000UL && pB <= 23000UL)
  {
    float newFreqB = 1000000.0 / (float)pB;

    frequencyBufferB[freqIndexB] = newFreqB;
    freqIndexB++;

    if (freqIndexB >= FREQUENCY_SAMPLES)
    {
      freqIndexB = 0;
    }
    ...
    for (byte i = 0; i < FREQUENCY_SAMPLES; i++)
    {
      sumB += frequencyBufferB[i];
    }
    freqB = sumB / FREQUENCY_SAMPLES;
  }
}
...
void setup()
{
  ...
  for (byte i = 0; i < FREQUENCY_SAMPLES; i++)
  {
    frequencyBufferA[i] = NOMINAL_FREQUENCY;
    frequencyBufferB[i] = NOMINAL_FREQUENCY;
  }
